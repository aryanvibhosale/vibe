...
class AudioFeatureProcessingPacker:
    ...
    def __init__(self, dataset_cnt: int, max_len: int, patch_size: int, feat_dim: int, audio_vae: nn.Module):
        ...
        hop_length = audio_vae.downsampling_ratio
        self.patch_size = patch_size
        self.patch_len = hop_length * self.patch_size
        ...
        self.verbose = False  # Suppress debug prints during training

    # ------------------------------------------------------------------ #
    # Helpers
    # ------------------------------------------------------------------ #
    @staticmethod
    def _first_pad_position(tokens: torch.Tensor, stereo: bool = False) -> int:
        positions = (tokens == -100).nonzero(as_tuple=True)
        if stereo:
            # In Songbloom, audio tokens are stereo, so we need to check both channels for padding
            positions = (tokens[0, :] == -100).nonzero(as_tuple=True), (tokens[1,:] == -100).nonzero(as_tuple=True)  
            if positions[1][0].numel() == 0:
                return None
            return int(positions[1][0][0])
    ...
    def unpad_audio_tokens(self, tokens: torch.Tensor):
        pad_pos = self._first_pad_position(tokens, stereo=True)
        to_return = tokens if pad_pos is None else tokens[:, :pad_pos]
        return to_return

    def encode_audio(self, wav: torch.Tensor, is_latent: bool = False):
        if is_latent:
            if wav.dim() == 2:
                wav = wav.unsqueeze(0)
            return wav.transpose(1, 2).to(torch.float32)
        
        chunk = wav.unsqueeze(0)  # Add batch dimension: [1, 2, chunk_num_samples]
        if self.verbose:
            print(f"Input audio chunk shape to AudioVAE: {chunk.shape}")
        wav_len = wav.size(-1)
        ...
        if self.verbose:
            print(f"Encoding audio chunk with shape: {chunk.shape}")
        with torch.no_grad():
            enc = self.audio_vae.encode(chunk)     # Diffusers returns a model output structure

        z_mean = enc.transpose(1,2)  # [B, 64, L/1024]
        return z_mean
    ...
    def __call__(
        ...
        if self.verbose:
            print("All audio tokens shape:", audio_tokens.shape)
        for idx, (audio_token, text_token, task_id, dataset_idx, is_prompt) in enumerate(zip(
            audio_tokens, text_tokens, task_ids.tolist(), dataset_ids.tolist(), is_prompts
        )):
            if self.verbose:
                print("Audio token shape before unpad:", audio_token.shape)
            ...
            is_latent = is_latents[idx] if is_latents else False
            ...
            (
                ...
                audio_duration,
                text_token_count,
            ) = self.process_functions[usage](unpad_audio_token, unpad_text_token, is_prompt, is_latent=is_latent)
            if self.verbose:
                print(f"Unpad Audio token shape", unpad_audio_token.shape)
    ...
    # ------------------------------------------------------------------ #
    def extract_audio_feats(self, audio_data: torch.Tensor, is_latent: bool = False):
        audio_feats = self.encode_audio(audio_data, is_latent=is_latent)
        
        if is_latent:
            assert audio_feats.size(1) % self.patch_size == 0, \
                f"Generated latent sequence length {audio_feats.size(1)} must be strictly divisible by patch_size {self.patch_size}."
    ...
    def process_tts_data(self, audio_token: torch.Tensor, text_token: torch.Tensor, is_prompt: bool = False, is_latent: bool = False):
        ...
        audio_feat_info, audio_duration = self.extract_audio_feats(audio_token, is_latent=is_latent)
