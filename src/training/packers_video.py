...
class AudioFeatureProcessingPackerForVideoInput:

    def __init__(self, dataset_cnt: int, max_len: int, patch_size: int, n_video_frames: int, feat_dim: int, audio_vae: nn.Module):
    ...
    def process_tts_data(self, audio_token: torch.Tensor, text_token: torch.Tensor, is_prompt: bool = False):
        ...
        text_token_info = torch.cat(
            [
                text_token,
                torch.zeros((self.n_video_frames,), dtype=torch.int32, device=text_token.device), # Add video frame placeholders
                torch.tensor(
        ...
        strict_text_length = text_token.shape[0]
