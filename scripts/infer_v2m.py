...
from core_video import VIBEVideo


def parse_args():
    parser = argparse.ArgumentParser("VIBE full-finetune inference (optionally with LoRA)")
    parser.add_argument(
        "--ckpt_dir",
        type=str,
        required=True,
        help="Base checkpoint directory (contains pytorch_model.bin, config.json, audiovae.pth, etc.)",
    )
    parser.add_argument(
        "--lora_weights_path",
        type=str,
        default="",
        help="Optional: path to a LoRA checkpoint directory (contains lora_weights.safetensors / lora_config.json)",
    )
    parser.add_argument(
        "--text",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Reference video path for music generation",
    )
    parser.add_argument(
        "--prompt_audio",
        type=str,
...
def main():
    ...
    lora_weights_path = args.lora_weights_path if args.lora_weights_path else None
    if lora_weights_path:
        print(f"[FT Inference] LoRA weights: {lora_weights_path}", file=sys.stderr)
    print(f"[FT Inference] Loading model: {args.ckpt_dir}", file=sys.stderr)
    model = VIBEVideo.from_pretrained(
        hf_model_id=args.ckpt_dir,
        baselm_path = f"{args.baselm_path}",
        audiovae_path = f"{args.audiovae_path}",
        load_denoiser=False,
        optimize=False,  # disable optimization for better generation quality
